...
class Notice {
	...
	public function prad_dashboard_banner_notice() {
		$prad_db_nonce  = wp_create_nonce( 'flynpo-dashboard-nonce' );
				...
				if ( $current_time >= $notice_start && $current_time <= $notice_end && $notice['visibility'] ) {
					...
					<div class="prad-notice-wrapper notice wc-install prad-free-notice">
						<div class="wc-install-body prad-image-banner">
							...
							?>
							"><?php esc_html_e( 'Dismiss', 'flyn-product-options' ); ?></a>
	...
	public function other_plugin_install_notice_callback( $type = '' ) {
		...
		?>
			<div class="prad-pro-notice prad-wc-install wc-install">
				<img width="100" src="<?php echo esc_url( FLYNPO_URL . 'assets/img/woocommerce.png' ); ?>" alt="logo" />
				<div class="prad-install-body">
					<h3><?php esc_html_e( 'Welcome to WowAddons.', 'flyn-product-options' ); ?></h3>
					<p><?php esc_html_e( 'WowAddons is a WooCommerce-based plugin. So you need to installed & activate WooCommerce to start using WowAddons.', 'flyn-product-options' ); ?></p>
					<div class="prad-install-btn-wrap">
						<a class="wc-install-btn prad-install-btn button button-primary" data-plugin-slug="woocommerce" href="#"><span class="dashicons dashicons-image-rotate"></span><?php file_exists( WP_PLUGIN_DIR . '/woocommerce/woocommerce.php' ) ? esc_html_e( ' Activate WooCommerce', 'flyn-product-options' ) : esc_html_e( ' Install WooCommerce', 'flyn-product-options' ); ?></a>
						<?php if ( 'required' !== $type ) : ?>
							<a href="<?php echo esc_url( add_query_arg( array( 'prad_install_key' => $install_key_tran ) ) ); ?>" class="prad-install-cancel wc-dismiss-notice">
								<?php esc_html_e( 'Discard', 'flyn-product-options' ); ?>
							</a>
						<?php endif; ?>
					</div>
	...
	public function install_activate_plugin() {
		if ( ! isset( $_POST['install_plugin'] ) || ! current_user_can( 'manage_options' ) ) {
			return wp_send_json_error( esc_html__( 'Invalid request.', 'flyn-product-options' ) );
		}
		...
		return wp_send_json_success( admin_url( 'admin.php?page=flynpo-dashboard#dashboard' ) );
	}
