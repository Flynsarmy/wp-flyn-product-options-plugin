...
class RequestApi {
	...
	public function get_option_callback( \WP_REST_Request $request ) {
		...
		if ( ! $post ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Post not found.', 'flyn-product-options' ),
				),
				404
			);
		}
	...
	public function set_option_callback( \WP_REST_Request $request ) {
		...
		$content = isset( $params['content'] ) && is_array( $params['content'] ) ? flynpo_product_options()->sanitize_rest_params( $params['content'] ) : '';
		$css     = isset( $params['css'] ) ? flynpo_product_options()->sanitize_rest_params( $params['css'] ) : '';
		...
		if ( 'new' === $id ) {
			...
			if ( is_wp_error( $id ) ) {
				return new WP_REST_Response(
					array(
						'success' => false,
						'message' => __( 'Failed to create a new option.', 'flyn-product-options' ),
					),
					400
				);
			}
			...
			return new WP_REST_Response(
				array(
					'success' => true,
					'message' => __( 'New option added.', 'flyn-product-options' ),
					'id'      => $id,
				),
				200
			);
			...
			if ( is_wp_error( $update ) ) {
				return new WP_REST_Response(
					array(
						'success' => false,
						'message' => __( 'Failed to update the option.', 'flyn-product-options' ),
					),
					400
				);
			}
			...
			return new WP_REST_Response(
				array(
					'success' => true,
					'content' => $content,
					'message' => __( 'Option updated.', 'flyn-product-options' ),
				),
				200
			);
		}
	...
	public function list_update_callback( \WP_REST_Request $request ) {
		...
		if ( empty( $ids ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'No IDs provided.', 'flyn-product-options' ),
				),
				400
			);
		}
				...
				return new WP_REST_Response(
					array(
						...
						'message' => sprintf(
							...
							__( 'Failed to update post ID: %1$s. Error: %2$s', 'flyn-product-options' ),
							$id,
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'status'  => $status,
				'message' => __( 'Items updated successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function list_delete_callback( \WP_REST_Request $request ) {
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Items deleted successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function list_duplicate_callback( \WP_REST_Request $request ) {
		...
		if ( empty( $id ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'No ID provided.', 'flyn-product-options' ),
				),
				400
			);
		}
			...
			return new WP_REST_Response(
				array(
					...
					'message' => sprintf(
						...
						__( 'Failed to duplicate post ID: %s.', 'flyn-product-options' ),
						$id
					),
				),
				400
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Item duplicated successfully.', 'flyn-product-options' ),
				'new_id'  => $new_id,
			),
			200
		);
	...
	public function list_import_callback( \WP_REST_Request $request ) {
		...
		$content = isset( $params['content'] ) && is_array( $params['content'] ) ? flynpo_product_options()->sanitize_rest_params( $params['content'] ) : array();
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Imported successfully.', 'flyn-product-options' ),
				'new_id'  => $new_id,
			),
			200
		);
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				$data[]            = array(
					...
					'assigned' => flynpo_product_options()->get_assigned_product_data( $id ),
				);
			}
	...
	public function assign_search_callback( \WP_REST_Request $request ) {
		...
		switch ( $trigger_type ) {
			case 'products':
				$response_data = flynpo_product_options()->get_searched_products( $search_keyword, false, $limit );
				break;
			case 'cat':
			case 'tag':
			case 'brand':
				$response_data = flynpo_product_options()->get_searched_categories(
					array(
						'term'         => $search_keyword,
						'limit'        => $limit,
						'includes'     => '',
		...
		if ( empty( $assigned_data ) || ! isset( $assigned_data['aType'] ) ) {
			return new WP_REST_Response(
				array(
					'success'   => false,
					'published' => false,
					'message'   => __( 'Invalid assigned data.', 'flyn-product-options' ),
				),
				400
			);
		}
	...
	public function get_assign_product_callback( \WP_REST_Request $request ) {
		...
		if ( $option_id ) {
			return new WP_REST_Response(
				array(
					'success'  => true,
					'assigned' => flynpo_product_options()->get_assigned_product_data( $option_id ),
				),
				200
			);
		}
	...
	public function set_assign_product_callback( \WP_REST_Request $request ) {
		...
		$product_image = ! empty( $params['product_image'] ) ? flynpo_product_options()->sanitize_rest_params( $params['product_image'] ) : array();
		...
		if ( empty( $option_id ) ) {
			return new WP_REST_Response(
				array(
					'success'  => false,
					'response' => array(
						'message' => __( 'No ID found', 'flyn-product-options' ),
					),
				),
				400
			);
		...
		return new WP_REST_Response(
			array(
				...
				'response' => array(
					...
					'option_id'     => $option_id,
					'message'       => __( 'Option Assigned Updated successfully', 'flyn-product-options' ),
					'newData'       => json_decode( flynpo_product_options()->safe_stripslashes( get_post_meta( $option_id, 'prad_base_assigned_data', true ) ), true ),
				),
			),
			200
		);
	...
	public function set_global_callback( \WP_REST_Request $request ) {
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Style saved successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function set_settings_callback( \WP_REST_Request $request ) {
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Settings saved successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function prad_handle_upload_field_mimes( $mimes ) {
		$prad_mimes = flynpo_product_options()->prad_get_upload_allowed_file_types();
	...
	protected function get_uploaded_file() {
		...
		if ( empty( $_FILES['prad_file'] ) ||
			...
			return new \WP_Error( 'no_file', __( 'No file found.', 'flyn-product-options' ) );
		}
		...
		if ( (int) $file['size'] > $max_file_size ) {
			return new \WP_Error(
				'file_size',
				__( 'File size exceeds the maximum allowed limit (25MB).', 'flyn-product-options' )
			);
		}

		$allowed_types = flynpo_product_options()->prad_get_upload_allowed_file_types();
		...
		if ( empty( $filetype['ext'] ) || empty( $filetype['type'] ) ) {
			...
			return new \WP_Error(
				'invalid_type',
				__( 'Invalid file type.', 'flyn-product-options' )
			);
		}
	...
	public function set_analytics_data_callback( \WP_REST_Request $request ) {
		...
		if ( $option_id && $type ) {
			...
			return new WP_REST_Response(
				array(
					...
					'option_id' => $option_id,
					'type'      => $type,
					'message'   => __( 'Analytics data updated.', 'flyn-product-options' ),
				),
				200
			);
		}
		return new WP_REST_Response(
			array(
				'success' => false,
				'message' => __( 'Invalid Analytics data.', 'flyn-product-options' ),
			),
			400
		);
	}
	...
	public function hello_bar_callback( \WP_REST_Request $request ) {
		...
		if ( 'hello_bar' === $type ) {
			flynpo_product_options()->set_transient_without_cache( 'prad_helloBar_newyr26', 'hide', $duration );
		}

		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Hello Bar Action performed', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function upload_font_callback( \WP_REST_Request $request ) {
		...
		if (
			...
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'No file found or invalid nonce.', 'flyn-product-options' ),
				),
				400
			);
		}
		...
		if ( empty( $font_title ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Font title is required.', 'flyn-product-options' ),
				),
				400
			);
		}
		...
		if ( $uploaded_file['size'] > $max_file_size ) {
			$file_error = __( 'File size exceeds the maximum allowed limit (10MB).', 'flyn-product-options' );
		}

		// Validate file extension.
		$file_extension = strtolower( pathinfo( $uploaded_file['name'], PATHINFO_EXTENSION ) );
		if ( ! in_array( $file_extension, $allowed_extensions, true ) ) {
			$file_error = __( 'Invalid file extension. Allowed types are: woff, woff2, ttf.', 'flyn-product-options' );
		}
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Font uploaded successfully.', 'flyn-product-options' ),
				'data'    => $font_data,
			),
			200
		);
	...
	public function delete_font_callback( \WP_REST_Request $request ) {
		...
		if ( empty( $font_id ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Font ID is required.', 'flyn-product-options' ),
				),
				400
			);
		}
		...
		if ( ! $font_found ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Font not found.', 'flyn-product-options' ),
				),
				404
			);
		}
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Font deleted successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
	...
	public function update_font_callback( \WP_REST_Request $request ) {
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Font updated successfully.', 'flyn-product-options' ),
			),
			200
		);
	}
