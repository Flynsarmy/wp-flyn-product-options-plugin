 ...
 * @since v.1.0.0
 */
namespace Flyn\ProductOptions\Order;

use Flyn\ProductOptions\Common\SafeMathEvaluator;
use Flyn\ProductOptions\Compatibility\BaseCurrency;
...
class CartPage {
	...
	public function save_custom_meta_to_cart( $cart_item_data, $product_id, $variation_id ) {
		$prad_selection = isset( $_POST['prad_selection'] ) ? flynpo_product_options()->sanitize_rest_params( $_POST['prad_selection'] ) : ''; //phpcs:ignore
		$option_ids     = isset( $_POST['prad_option_published_ids'] ) ? flynpo_product_options()->sanitize_rest_params( json_decode( wp_unslash( $_POST['prad_option_published_ids'] ), true ) ) : array();//phpcs:ignore
		$prad_products_selection = isset( $_POST['prad_products_selection'] ) ? flynpo_product_options()->sanitize_rest_params( $_POST['prad_products_selection'] ) : ''; // phpcs:ignore
		$prad_products_selection = json_decode( flynpo_product_options()->safe_stripslashes( $prad_products_selection ), true );
	...
	public function calculate_option_price( $prad_selection, $product_id, $option_ids, $variation_id = '' ) {
		...
		try {
			...
			if ( ! ( is_array( $option_ids ) && ! empty( $option_ids ) ) ) {
				$option_all = json_decode( flynpo_product_options()->safe_stripslashes( get_option( 'prad_option_assign_all', '[]' ) ), true );        // do for all cat , product.
				$option_all = is_array( $option_all ) ? $option_all : array();

				$option_product = json_decode( flynpo_product_options()->safe_stripslashes( get_post_meta( $product_id, 'prad_product_assigned_meta_inc', true ) ), true );
				$option_product = is_array( $option_product ) ? $option_product : array();

				$option_exclude = json_decode( flynpo_product_options()->safe_stripslashes( get_post_meta( $product_id, 'prad_product_assigned_meta_exc', true ) ), true );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					if ( $terms && ! is_wp_error( $terms ) ) {
						foreach ( $terms as $term ) {
							$meta_inc = json_decode( flynpo_product_options()->safe_stripslashes( get_term_meta( $term->term_id, 'prad_term_assigned_meta_inc', true ) ), true );
			...
			$prad_cart_item_selection = json_decode( flynpo_product_options()->safe_stripslashes( $prad_selection ), true );
						...
						if ( isset( $field['type'] ) && 'upload' === $field['type'] ) {
							...
							if ( isset( $field['value']['path'] ) ) {
								if ( $this->is_image_url( $field['value']['path'] ) ) {
									...
									$res = wp_kses( '<span><strong>' . __( 'Name', 'flyn-product-options' ) . ': </strong>' . wp_kses_post( $field['value']['name'] ) . '</span><span> <strong>' . __( 'Path', 'flyn-product-options' ) . ':</strong> ' . wp_kses_post( $field['value']['path'] ) . '</span>', $prad_allowed_html_tags );
								}
								...
								foreach ( $field['value'] as $item ) {
									...
									if ( isset( $item['label'] ) ) {
										...
										$count  = wp_kses( '<span> <strong>' . __( 'Count', 'flyn-product-options' ) . ':</strong> ' . $count_val . '</span>', $prad_allowed_html_tags );
						...
						if ( isset( $field['_vDatas'] ) ) {
							foreach ( $field['_vDatas'] as $i => $index ) {
								if ( isset( $option_data[ $index ] ) ) {
									$cost   = ( isset( $option_data[ $index ]->sale ) && $option_data[ $index ]->sale ) ? $option_data[ $index ]->sale : $option_data[ $index ]->regular;
									...
									if ( isset( $f_value['type'] ) ) {
										if ( 'number' === $f_value['type'] || 'range' === $f_value['type'] ) {
											...
											if ( $selected_option_index !== '' ) {
												...
												$select_block_cost   = ( isset( $select_block_data[ $selected_option_index ]->sale ) && $select_block_data[ $selected_option_index ]->sale ) ? $select_block_data[ $selected_option_index ]->sale : $select_block_data[ $selected_option_index ]->regular;
							...
							$cost   = ( isset( $option_data[0]->sale ) && $option_data[0]->sale ) ? $option_data[0]->sale : $option_data[0]->regular;
