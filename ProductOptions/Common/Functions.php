...
class Functions {
		...
		if ( isset( $item->variation ) && $item->variation ) {
			...
			foreach ( $available_variations as $variation_data ) {
				...
				if ( $variation && $variation->is_purchasable() && $variation->is_in_stock() ) {
					...
					$price_obj     = flynpo_product_options()->get_price_object( $regular_price, $sale_price );
			...
			if ( $select_options ) {
				...
				<div class="prad-product-block-variation-select prad-mt-10">
					<div class="prad-custom-select prad-w-full prad-product-variation-select-comp">
						<div class="prad-select-box prad-block-input prad-block-content" readonly="readonly"><div style="max-width: 120px" class="prad-select-box-item prad-mr-12 prad-ellipsis"><?php esc_html_e( 'Select an option', 'flyn-product-options' ); ?></div> <div class="prad-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="8" fill="none"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="m1 1 6 6 6-6"></path></svg></div></div>
	...
	function generate_utm_link( $params ) {
		...
		$base_url      = $params['url'] ?? 'https://www.wpxpo.com/flyn-product-options-for-woocommerce/';
