...
namespace Flyn\ProductOptions\Common;
...
class Hooks {
	...
	public function handle_prad_load_script_on_ajax() {
		...
		$front_script = FLYNPO_URL . 'assets/js/frontend-script.js';
		echo '<script src="' . esc_url( $front_script ) . '" id="prad-front-script-js-2"></script>';

		$data_script = FLYNPO_URL . 'assets/js/wowdate-min.js';
		echo '<script src="' . esc_url( $data_script ) . '" id="prad-front-date-js-2"></script>';

		$flag_script = FLYNPO_URL . 'assets/js/wowflag.js';
		echo '<script src="' . esc_url( $flag_script ) . '" id="prad-flag-script-js-2"></script>';

		// Load Needed CSS.
		$frontend_css = file_get_contents( FLYNPO_URL . 'assets/css/wowaddons-frontend.css' );
		echo '<style id="prad-frontend-css-inline">' . wp_strip_all_tags( $frontend_css ) . '</style>';

		$block_css = file_get_contents( FLYNPO_URL . 'assets/css/wowaddons-blocks.css' );
	...
	public function enqueue_block_js_callback() {
		wp_enqueue_script( 'prad-front-script', FLYNPO_URL . 'assets/js/frontend-script.js', array( 'wp-api-fetch', 'jquery', 'wp-i18n' ), FLYNPO_VER, true );
		wp_enqueue_script( 'prad-date-script', FLYNPO_URL . 'assets/js/wowdate-min.js', array( 'jquery' ), FLYNPO_VER, true );
		wp_enqueue_script( 'prad-flag-script', FLYNPO_URL . 'assets/js/wowflag.js', array( 'jquery' ), FLYNPO_VER, true );
		wp_localize_script(
			'prad-front-script',
			'prad_option_front',
			$this->get_prad_option_front_data()
		);
		wp_set_script_translations( 'prad-front-script', 'flyn-product-options', FLYNPO_PATH . 'languages/' );
	}
	/**
	...
	public function enqueue_block_css_callback() {
		wp_enqueue_style( 'prad-frontend-css', FLYNPO_URL . 'assets/css/wowaddons-frontend.css', array(), FLYNPO_VER );
	...
	public function get_prad_option_front_data() {
		return array_merge(
			...
			flynpo_product_options()->get_currency_converted_data(),
		);
	}
}
