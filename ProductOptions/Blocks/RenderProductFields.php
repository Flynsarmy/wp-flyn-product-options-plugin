...
namespace Flyn\ProductOptions\Blocks;

use Flyn\ProductOptions\Blocks\Renderers\BlockRenderer;
use Flyn\ProductOptions\Services\ProductBlocks;
...
class RenderProductFields {
	 ...
	 * @var ProductBlocks
	 */
	private ProductBlocks $blocks_service;
	...
	public function __construct() {
		...
		$this->blocks_service = new ProductBlocks();
		...
		foreach ( $blocks_data['blocks'] as $addon_id => $addon_blocks ) {
			...
			if ( current_user_can( Xpo::prad_old_view_permisson_handler() ) ) {
				$html .= sprintf(
					...
					esc_url( admin_url( 'admin.php?page=flynpo-dashboard#lists/' . $addon_id ) ),
					esc_html__( 'Edit Addon', 'flyn-product-options' )
				);
			}
