...
class ProductsBlock extends AbstractBlock {
	...
	private function get_product_data( int $product_id, bool $with_variations ) {
		return \flynpo_product_options()->get_product_block_product_attr( $product_id, $with_variations );
	}
	...
	private function get_variation_html( $item, int $index ): string {
		return \flynpo_product_options()->generate_products_block_variation_section_html(
			array(
				'item'  => $item,
				'index' => $index,
			)
	...
	private function render_swatch_item( $item, int $index, string $variation_html ): string {
		$price_info  = flynpo_product_options()->get_price_object( $item->regular, $item->sale );
		$layout      = $this->get_property( 'layout', '_default' );
	...
	private function render_swatch_label( $item, int $index ): string {
		$blockid = $this->get_block_id();
		$img_url = isset( $item->img ) ? $item->img : FLYNPO_URL . 'assets/img/default-product.svg';
