...
namespace Flyn\ProductOptions\Services;
...
class ProductBlocks {
	...
	private function get_product_option_ids( int $product_id ): array {
		...
		// Remove duplicates and respect merge order.
	...
	private function maybe_render_addon_css( int $addon_id ): void {
		...
		if ( function_exists( 'product_addons' ) ) {
			flynpo_product_options()->render_addon_css( $addon_id, $print_styles );
		}
	...
	private function get_json_option( string $option_name, array $default = array() ): array {
		...
		$decoded = json_decode( flynpo_product_options()->safe_stripslashes( $value ), true );
