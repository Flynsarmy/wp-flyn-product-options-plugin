( function ( $ ) {
	...
	if ( pradMenuHref?.indexOf( '?page=flynpo-dashboard' ) > 0 ) {
	...
	if ( '?page=flynpo-dashboard' == window.location.search ) {
	...
	$( '#toplevel_page_prad-dashboard ul > li > a' ).each( function () {
		if (
			$( this ).attr( 'href' ) &&
			$( this ).attr( 'href' ).indexOf( '?page=flynpo-dashboard' ) > 0
		) {
			...
			$( this ).attr(
				...
				$( this )
					...
					.replace(
						...
						'admin.php?page=flynpo-dashboard#dashboard'
					)
			);
		}
	} );
	...
	if ( $( '#plugin-information-content' ).length > 0 ) {
		$(
			...
			'href',
			'https://www.wpxpo.com/flyn-product-options-for-woocommerce/'
		);
	}
} )( jQuery );
